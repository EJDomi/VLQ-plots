...
void DrawPlot( TCanvas * c1, TH1D * hBack, TH1D * hData, std::map<TString,TH1D*> &hSignal, TH1D * hQCD = 0, TH1D * histW = 0, TH1D * histTT = 0, TH1D * histST = 0, bool signalOnly = 0 )
{
	...
	if(hBack && !signalOnly) {
	   if(hBack->GetMaximumBin() > hBack->GetXaxis()->GetNbins() * .4){
        ...
        TLegend * leg = new TLegend(x0,0.6,x1,0.85,"","brNDC");
	...
	if ( hSignal.size() > 0 ) {
		...
		it->second->Draw("hist same"); 
                if (it->first == "Tprime1000_LH") leg->AddEntry(it->second,"M(T) = 1000 GeV, LH");
                else if ( it->first == "Tprime1200_LH") leg->AddEntry(it->second,"M(T) = 1200 GeV, LH");
                else if ( it->first == "Tprime1500_LH") leg->AddEntry(it->second,"M(T) = 1500 GeV, LH");
                else if ( it->first == "Tprime1800_LH") leg->AddEntry(it->second,"M(T) = 1800 GeV, LH");
                else if ( it->first == "Tprime1200_RH") leg->AddEntry(it->second,"M(T) = 1200 GeV, RH");
                else if ( it->first == "Tprime1800_RH") leg->AddEntry(it->second,"M(T) = 1800 GeV, RH"); 
		double integral, err;
...
*/
	...
	latex.DrawLatex(0.725,.89,cmsText);
